...
var imgSrcHead = "http://location.gene-point.com";
...
var canvasScale = 1.0;
var customerScale = 1.0;
function getAllPersonRealtimePosition(requestData){
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/customer/get_person_realtime",
		...
		success : function(result){
			if (result.status !== 200){
	        	...
	        	var contextbuffer = canvasbuffer.getContext('2d');
	        	 getMap(requestData.floor);
                imgBG.src = imgSrcHead + json[posBuilding].floors[posFloor].mapUrl; // 设置地图
	        	...
	        	imgBG.onload = function(){
	        		...
	        		if(width>maxWidth){
	        			customerScale = maxWidth*1.0/width;
	        		}else{
	        			customerScale = 1.0;
					}
	        		
	        		$("#customer_map").css("background","url("+imgBG.src+") no-repeat");
					$("#customer_map").css("width",width * canvasScale * customerScale);
					$("#customer_map").css("height",height * canvasScale * customerScale);
					//关键设置，不然图片不会等比例缩放
					$("#customer_map").css("background-size","contain");
					
	        		canvasbuffer.width = width * canvasScale * customerScale ;
	        		canvasbuffer.height = height * canvasScale * customerScale;
	        		canvas.width = width * canvasScale * customerScale;
	        		canvas.height = height * canvasScale * customerScale;
	        		context.drawImage(imgBG, 0, 0, width * canvasScale * customerScale,
	        				height * canvasScale * customerScale);
	        ...
	        //			var index = 1;
	        			...
	        			if (macData == '') {
	        				for(var i in data){
		        				contextbuffer.drawImage(imgMarker,data[i].corx * canvasScale* customerScale,data[i].cory * canvasScale* customerScale);
		        			}
						    ...
						    for(var j in macData){
						    	for(var i in data){
						    		 if(macData[j] != data[i].mac){
						    			 ...
						    			 posX = data[i].corx * canvasScale* customerScale;
						    			 posY = data[i].cory * canvasScale* customerScale;
						    		 }
						    	 }	
						    	 if(posX != 0){
...
function getTopKMAC(requestData){
    $.ajax({
        type : "GET",
        dataType : "json",
        url : basePath + "/track/get_topk_mac?" + new Date(),
        contentType : "application/x-www-form-urlencoded; charset=UTF-8",
        data : {
            "building" : requestData.building
        },
...
function getUserHistoryTrack(requestData){
    ...
    $.ajax({
        type : "GET",
        dataType : "json",
        url : basePath + "/track/get_track_history?" + new Date(),
        contentType : "application/x-www-form-urlencoded; charset=UTF-8",
        data : {
            "building" : requestData.building,
            "mac"      : requestData.mac,
			...
			"durationEnd" : requestData.duration_end,
			"durationStart" : requestData.duration_start
        },
        beforeSend : function(){
...
function getTrackPlayback(requestData,playSpeed){
		...
		type : "GET",
		dataType : "json",
		url : basePath + "/track/get_track_trend?" + new Date(),
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		data : {
			"building" : requestData.building,
			"floor"    : requestData.floor,
		...
		success : function(result){
			...
			if (result.status !== 200){
				...
				if(img.complete){
					var width = img.width;
					var height = img.height;
					var maxWidth = $('#collapse-panel-2').width();
					if(width>maxWidth){
						MoreScale = maxWidth*1.0/width;
					}else{
						MoreScale = 1.0;
					}
					width *= MoreScale;
					height *= MoreScale;
					showTrackPlayback(data,img,playSpeed,width,height);
				}else{
					img.onload = function(){
						var width = img.width;
						var height = img.height;
...
function drawTrackNoDelay(groupData,groupIndex,img,context){
    ...
    var p = index;
    var step = 0;
    var newData= JSON.parse(groupData);
    ...
    for(var i in data){
        //画点
        context.arc(data[index].corx, data[index].cory, radius, 0, 2 * Math.PI);
        context.fill();
        //画线
        if(index>0){
            context.beginPath();
            context.moveTo(data[index-step].corx, data[index-step].cory);
            context.lineTo(data[index].corx, data[index].cory);
            context.stroke();
            context.closePath();
        }else{
            context.beginPath();
            context.moveTo(data[0].corx, data[0].cory);
            context.closePath();
        }
        // 随机跳跃打点
...
var abortDraw = false;
var MoreScale;
function showTrackPlayback(data,img,playSpeed,width,height){
	$('#trackWithHeatmap').html('');
	$('#trackWithHeatmap').css("background","url("+img.src+") no-repeat");
	$('#trackWithHeatmap').css("width",width);
	$('#trackWithHeatmap').css("height",height);
	// $('#trackWithHeatmap').html('');
	$("#trackWithHeatmap").css("background-size","contain");
	...
	var drawHandle = setInterval(function(){
		...
		drawPointsWithD3(data[total].data,img,"trackWithHeatmap",width,height);
...
function drawPointsWithD3(data,img,domId,width,height){
	/*var w = img.width-50;
	var h = img.height-50;*/
	var w = width-50;
	var h = height-50;
	...
	for(var i=0;i<data.length;i++){
		dataset.push([data[i].x*MoreScale,data[i].y*MoreScale]);
	}
...
function getAllApPosition(requestData,mapWidth,mapHeight,imgMarker,mapScale){
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/ap/get_ap_position",
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		data : {
			"building" : requestData.building,
			"floor" : requestData.floor
...
function drawimgMarker(canvas, context, canvasbuffer, contextbuffer, floor, positions,imgMarker){	    
		for ( var j in positions){
			contextbuffer.drawImage(imgMarker, JSON.stringify(positions[j].x-8)* canvasScale, JSON.stringify(positions[j].y-22)* canvasScale);
			// ap扫描到的手机数量
...
function getMap(floor) {
   json = userinfo.buildings;
    for (var i = 0, len1 = json.length; i < len1; i++) {
        if (json[i].buildingCodeAuto == building) {
            posBuilding = i;
        }
    }

    var len2 = json[posBuilding].floors.length;

    for (var j = 0; j < len2; j++) {
        if (json[posBuilding].floors[j].floor == floor) {
            posFloor = j;
        }
    }
}
