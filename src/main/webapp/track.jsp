...
function getMapPathAndForbiddenRegionData(requestData){
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/track/get_map_path_forbidden_region?" + new Date(),
...
new scale('btn0', 'bar0', 'title0');   
	...
	function getPersonHistoryTrack(requestData){
		$.ajax({
			type : "GET",
			dataType : "json",
			url : basePath +"/track/get_history_person_track?" + new Date(),
			contentType : "application/x-www-form-urlencoded; charset=UTF-8",
			data : {
				 "building" : requestData.building,
		         "mac"      : requestData.mac,
		...
		var res = result.data;		
		var data = res.data;
		 var floorOrder = res.floorOrder;
	  var allTimeDuration = res.allTimeDuration;	
		...
		for(var i in floorOrder){	
	        ...
	        var b;
		       for(var i in data){
		        ...
		        getMap(backData[0][0].floor);			   
		        imgBack.src = imgSrcHead + json[posBuilding].floors[posFloor].mapUrl;// 设置地图
				if(imgBack.complete){
		...
		timeQueryStart = res.timeQueryStart;
		timeQueryEnd = res.timeQueryEnd;
	...
	function drawChangeFloor(data, groupIndex,floorOrder){
		var img = new Image();
		getMap(data[groupIndex][0].floor);			   
...
/* 		img.src = basePath + "/static/map/" + building + "_"
	...
	function showDynamicFloorTrack(trackData, groupIndex ,img, floor,floorOrder) {
		...
		if (mapPaths.hasOwnProperty(floor)) {
			 trackData[groupIndex] = updateTrackPoints(trackData[groupIndex],floor);
