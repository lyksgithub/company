...
function getRealtimeCustomerShopData(requestData){
	$.ajax({
		type : "GET",
		dataType : "json",
		url :  basePath + "/customer/get_realtime_customer_shop",
...
function getShopList(requestData){
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/shop/get_shop_list?" + new Date(),
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		data : {
		...
		success : function(result) {
			if (result.status !== 200) {
				...
				for(var i in shopList){
					html += '<option value=' + shopList[i].shopId + ' floor="'+shopList[i].floor+'">'
					+ shopList[i].shopName+' ('+shopList[i].floor+')</option>';
					count++;
				}
...
function statisticRateInShop(requestData) {
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/shop/get_rate_in_shop?" + new Date(),
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		data : {
...
function statisticHistoryFlow(requestData) {
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/customer/get_customer_flow_history",
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		data : {
			"building" : requestData.building,
			"duration" : requestData.duration,
...
function statisticStayTimeInShop(requestData){
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/shop/get_staytime_in_shop?" + new Date(),
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		data : {
		...
		success : function(result) {
			...
			if (result.status !== 200) {
				...
				var data = result.data.stayTimeTypeTemp.split(",");
...
function getHistoryShopRank(requestData){
	$.ajax({
		type : "GET",
		dataType : "json",
		url : basePath + "/shop/get_shop_rank?" + new Date(),
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		data : {
			"building" : requestData.building,
			"time"     : requestData.time
